...
- `just fmt`: format Nix codebase using the flake formatter (`alejandra`).
...
- Run `just fmt` before committing. Pre-commit also enforces whitespace/YAML checks and runs `alejandra` on `*.nix`.
